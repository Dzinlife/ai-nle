...
import {
	...
	type OffscreenRenderContextValue,
} from "./OffscreenRenderContext";
...
const Preview = () => {
	 ...
	 * ```
	 */
	const renderElementsOffscreen = useCallback(
		async (
			...
			// 动态导入 SkiaSGRoot（因为它是内部模块，不在公共 API 中）
			// 注意: 这需要确保构建工具支持动态导入
			...
			// 创建 TimelineProvider 的 Context Bridge
			// const TimelineBridge = useContextBridge(TimelineContext);
	 ...
	 */
	const renderElementsOffscreenAsImage = useCallback(
		async (
			skiaReactElements: React.ReactElement[],
			size: SkSize,
			currentTime: number = 0,
